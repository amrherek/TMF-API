...
import com.lhs.ccb.common.soi.ParameterList.ParameterInfo;
...
public class RemoteBackendImpl implements SVLBackendPortType {
    ...
    public void execute(
        ...
        if ("THROW_SEC".equals(operation)) {
            ...
            try {
                throw new SecurityException("Throw by User {}", new String[]{localApiClientRequest.getBscsAuthentication().getBscsLogin()});
            ...
            if (null != svlInput && 0 < svlInput.length()) {
                try {
                    in = new SVLDeserializerIXR4().unmarshall("execute ANONYMOUS.COMMAND {" + svlInput + "}");
                } catch (IOException e) {
        ...
        try {
            if ("OPEN".equals(operation) || "CLOSE".equals(operation)) {
                ...
                SVLObjectWrapper wrapperIn = new SVLObjectWrapperIXR4(in);
...
//    			sb.append(right);
    ...
    private void fill(SOAParameterList soaPl, ParameterList pl) {
        for (String attrName : pl.getParameterNames()) {
            ParameterInfo pi = pl.getParameterInfo(attrName);
            ...
            soiPI.setMandatory(pi.isMandatory());
            soiPI.setPublicKey(pi.isPublicKey());
            soiPI.setPrivateKey(pi.isPrivateKey());
